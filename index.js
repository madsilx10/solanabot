...
async function swap(inputMint, outputMint, amount) {
  ...
  try {
    ...
    // Kalau no routes, token belum siap di-swap — skip semua fallback
    if (err.message.includes("No routes found")) {
      return { success: false, noRoute: true, error: "No routes found — token belum terdaftar di DEX manapun" };
    }
  }
