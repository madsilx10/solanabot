...
if (!PRIVATE_KEY || !TELEGRAM_BOT_TOKEN || !TELEGRAM_CHAT_ID || !OPENROUTER_API_KEY) {
...
async function checkRug(mint) {
  try {
    ...
    return {
      safe: redFlags.length === 0 && score < 5000,
      score,
    ...
    return { safe: true, score: 0, risks: [], redFlags: [] }; // kalau gagal, lanjut aja
  }
}
...
async function analyzeToken(tokenData) {
  const prompt = `You are a Solana meme coin crypto analyst. Analyze this token and decide if it is worth short-term trading.
...
{"verdict":"BUY or SKIP","score":1-10,"reason":"short reason","risk":"LOW/MEDIUM/HIGH","target":potential_upside_percent}`;
  ...
  for (const model of OPENROUTER_MODELS) {
    try {
      ...
      const data = await res.json();
      if (data.error?.code === 429 || data.error?.type === "insufficient_quota") {
        console.log(`[AI] ${model} rate limited, trying next...`);
        continue;
      }
      const text = data.choices?.[0]?.message?.content || "";
      if (!text) { console.log(`[AI] ${model} empty response`); continue; }
      const match = text.match(/{[\s\S]*}/);
      if (!match) { console.log(`[AI] ${model} no JSON`); continue; }
...
async function executeBuy(mint, symbol, analysis) {
...
async function scanLoop() {
  ...
  for (const token of tokens) {
    ...
    if (liq < 5000 || liq > 100000 || vol < 500) continue;
