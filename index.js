...
// Jupiter endpoint — public endpoint, no API key needed
const JUP_BASE = "https://public.jupiterapi.com";
const JUP_HEADERS = { "Content-Type": "application/json" };
...
async function swapJupiter(inputMint, outputMint, amount) {
  const quoteRes = await fetch(`${JUP_BASE}/quote?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amount}&slippageBps=1000&onlyDirectRoutes=true&maxAccounts=20`, { headers: JUP_HEADERS });
  ...
  const swapRes = await fetch(`${JUP_BASE}/swap`, {
    ...
    body: JSON.stringify({ quoteResponse: quote, userPublicKey: walletAddress, wrapAndUnwrapSol: true, dynamicComputeUnitLimit: true, prioritizationFeeLamports: 500000 }),
  });
...
async function swapJupiterV2(inputMint, outputMint, amount) {
  ...
  const quoteRes = await fetch(`${JUP_BASE}/quote?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amount}&slippageBps=2000&onlyDirectRoutes=false&maxAccounts=20`, { headers: JUP_HEADERS });
  const quoteText = await quoteRes.text();
...
async function swap(inputMint, outputMint, amount) {
  ...
  try {
    ...
    if (err.message.includes("No routes found")) noRoute = true;
  }
  ...
  if (noRoute) return { success: false, noRoute: true, error: "No routes found — token belum terdaftar di DEX manapun" };
...
async function executeBuy(mint, symbol) {
  ...
  if (!result.success) {
    if (result.noRoute) {
      console.log(`[BUY] Skip ${symbol}: no DEX route yet`);
      return false; // signal ke scanLoop buat lanjut ke token berikutnya
    }
    await sendTelegram(`❌ Buy gagal *${symbol}*: ${result.error}`);
    return false;
  }
...
async function scanLoop() {
  ...
  for (const pair of pairs) {
    ...
    if (!rug.safe) {
      console.log(`[RugCheck] SKIP ${symbol}: score=${rug.score} redFlags=${rug.redFlags.join(", ")}`);
      continue;
    }
    ...
    const bought = await executeBuy(mint, symbol);
    if (bought !== false) break;
  }
}
