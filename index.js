...
async function swapJupiter(inputMint, outputMint, amount) {
  const quoteRes = await fetch(`https://quote-api.jup.ag/v6/quote?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amount}&slippageBps=1000`);
...
async function swapJupiterV2(inputMint, outputMint, amount) {
  // Fallback: Jupiter dengan endpoint alternatif
  const quoteRes = await fetch(`https://quote-api.jup.ag/v6/quote?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amount}&slippageBps=2000&onlyDirectRoutes=false`);
  const quote = await quoteRes.json();
