...
async function executeBuy(mint, tokenInfo) {
  ...
  const randomTP = Math.floor(Math.random() * (100 - 20 + 1)) + 20;
  activePosition = { mint, symbol: tokenInfo.symbol, buyPrice, tokenAmount: result.outAmount, buyTime: Date.now(), tp: randomTP };

  await sendTelegram(
    `*Buy berhasil!*\nToken: *${tokenInfo.symbol}*\nBuy price: $${buyPrice.toFixed(8)}\nAmount: $${BUY_AMOUNT_USD}\nTP: +${activePosition.tp}% | SL: -${STOP_LOSS_PCT}%\nTX: \`${result.sig}\``,
