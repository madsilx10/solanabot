...
const { Connection, Keypair, VersionedTransaction, PublicKey, Transaction, LAMPORTS_PER_SOL } = require("@solana/web3.js");
...
const JUP_HEADERS = JUP_API_KEY
  ? { "Content-Type": "application/json", "Authorization": `Bearer ${JUP_API_KEY}` }
...
// PUMP.FUN fallback swap
function isPumpToken(mint) {
  return mint.endsWith("pump");
}

async function swapPumpFun(mint, isBuy, lamports) {
  try {
    ...
    if (isBuy) {
      ...
      // slippage 10%
      const result = await sdk.buy(keypair, mintPubkey, solAmount, BigInt(1000), {
        unitLimit: 250000,
        unitPrice: 250000,
      });
      if (!result.success) throw new Error("PumpFun buy failed");
      console.log("[Swap] PumpFun buy success:", result.signature);
      // estimasi token dapat: ambil dari hasil buy
      const tokenAmount = result.results?.tokenAmount || BigInt(0);
      return { sig: result.signature, outAmount: tokenAmount.toString() };
    } else {
      // sell: lamports di sini adalah token amount
      const tokenAmount = BigInt(lamports);
      const result = await sdk.sell(keypair, mintPubkey, tokenAmount, BigInt(0), {
        unitLimit: 250000,
        unitPrice: 250000,
      });
      if (!result.success) throw new Error("PumpFun sell failed");
      console.log("[Swap] PumpFun sell success:", result.signature);
      return { sig: result.signature, outAmount: "0" };
    }
  } catch (err) {
    throw new Error(`PumpFun swap error: ${err.message}`);
  }
}
...
async function swap(inputMint, outputMint, amount) {
  ...
  // Fallback PumpFun (khusus token pump.fun)
  ...
  return { success: false, error: "All swap methods failed (Jupiter + PumpFun)" };
}
